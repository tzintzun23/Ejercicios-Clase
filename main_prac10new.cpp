...
//Práctica: 10
//Versión VS: 2017
//Instrucciones: Se presenta un coche al que se le colocaron las
// llantas en la debida posición, además se agregó la rotación a
// estas. Por último se agregó la condición de paro del coche una
// vez que este llegaba al final del camino para que retrocediera
// y viceversa.
	...
	glPushMatrix();
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glPushMatrix();
					glTranslatef(-6.0, 0.0, -9.5);
					glRotatef(rotacionLlanta, -1, 0, 0);
					llanta.GLrender(NULL,_SHADED,1.0);
				glPopMatrix();

				//llanta delantera derecha
				glPushMatrix();
					glTranslatef(-6.0, 0.0, 7.5);
					glRotatef(rotacionLlanta, -1, 0, 0);
					llanta.GLrender(NULL, _SHADED, 1.0);
				glPopMatrix();
...
void animacion()
{
	...
	if(g_fanimacion)
	{
		if(avanza)
		{
			movKit += 0.5;
			printf("%f\n", movKit);
			rotacionLlanta = (rotacionLlanta - 4) % 360;
			if (movKit > 125.0)
				avanza = false;
		}
		else
